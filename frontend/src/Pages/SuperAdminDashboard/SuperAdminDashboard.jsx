    ...
    <Box sx={mainDiv}>
      ...
      <Box sx={stats}>
        <Cards
          ...
          info={`${
            orgData?.currentMonthCount?.count
              ? orgData?.currentMonthCount.count
              : "0"
        ...
        <Cards
          ...
          info={`${
            userData.currentMonthCount?.count
              ? userData.currentMonthCount.count
              : "0"
          } active admins this month`}
          color={"success"}
        />
        <Cards
          title={"Pending Complaints"}
          amount={
            complaintData.totalPendingCount
              ? complaintData.totalPendingCount
              : "0"
          }
          info={`${
            complaintData.currentMonthPendingCount
              ? complaintData.currentMonthPendingCount
              : "0"
        ...
        <Cards
          ...
          amount={
            complaintData.totalResolvedCount
              ? complaintData.totalResolvedCount
              : "0"
          }
          info={`${
            complaintData.currentMonthResolvedCount
              ? complaintData.currentMonthResolvedCount
              : "0"
      ...
      <Box sx={chartDiv}>
        {orgTable === 1 ? (
          <Box sx={{ width: "100%" }}>
            <BarChart
              data={mapData({
                data: orgData?.monthlyCount,
                label: [["Month", "Organizations"]],
              })}
            />
          </Box>
        ) : (
      ...
      <Box sx={tableDiv}>
        ...
        <Link className="admin-all-link" to="/complaints">
          See all
        </Link>
      </Box>
      <Tables
